...
import { STYLE_ROW, TIMESCALEH } from "../definitions/style";
...
export default class MainPane {
  ...
  get width() { return this.#elMain.width; }
  get height() { return this.#elMain.height; }
  get chartW() { return this.elPrimary.width; }
  get chartH() { return this.elPrimary.height; }
  get rowsW() { return this.#elRows.width; }
  get rowsH() { return this.#elRows.height; }
  get rowMinH() { return this.#rowMinH; }
  set rowMinH(h) { if (isNumber(h)) this.#rowMinH = Math.abs(h); }
  get dimensions() { return this.#elMain.dimensions; }
  get pos() { return this.#elMain.dimensions; }
  ...
  init(options) {
    ...
    options.parent = this;
    options.chartData = this.#core.allData.data;
    options.primaryPane = this.#core.allData.primaryPane;
    options.secondaryPane = this.#core.allData.secondaryPane;
    ...
    if (this.#core.theme?.time?.navigation === false) {
      ...
      this.#elRows.style.height = "1fr"; //`calc(100% - ${TIMESCALEH}px)`
    }
  ...
  start() {
    ...
    this.#ChartPanes.forEach((view, key) => {
      ...
      maxMin.panes[key] = view.element.clientHeight;
    });
  ...
  restart() {
    ...
    for (let [k, v] of this.views) {
      for (let i of v) {
        ...
        this.addIndicator(i.type, i.name, {
          data: i.data,
          settings: i.settings,
        });
      }
  ...
  onChartDrag(e) {
    ...
    this.#cursorPos =
      ("n", [e.position.x, e.position.y, ...d.start, ...d.delta]);
  ...
  setDimensions() {
    ...
    if (this.#ChartPanes.size == 1 && chartH != height) {
      this.#Chart.setDimensions({ w: width, h: height });
    } 
    // ensure that all pane heights tally to the rowsH
    else {
      const heights = []
      let diff = 0, height, totalH = 0;
      this.#ChartPanes.forEach((chartPane, key) => {
         height = Math.round(chartPane.height * resizeH)
         heights.push( {chartPane, height} )
         totalH += height
      })
      if ( totalH < this.rowsH ) {
        diff = Math.round((this.rowsH - totalH ) / this.#ChartPanes.size)
      }
      for (let h of heights) {
        h.chartPane.setDimensions({w: this.rowsW, h: h.height + diff})
      }
    }
...
// this.#elRows.width
  ...
  addChartPane(options) {
    ...
    // insert a new row for the primary or secondary chart (indicator)
    let row;
  ...
  removeChartPane(paneID) {
    if (!isString(paneID) || !this.#ChartPanes.has(paneID)) return false;
    ...
    if (chartPane.status !== "destroyed") chartPane.destroy();
    ...
    if (this.#ChartPanes.size === 1) {
      ...
      o.setDimensions({ w: undefined, h: this.#elRows.height });
    } else {
  ...
  addIndicator(i, name = i, params = {}) {
    ...
    // add secondary chart indicator
      ...
      const indType =
        indicator.primaryPane === "both" && isBoolean(i.primaryPane)
          ? i.primaryPane
          : false;
  ...
  calcChartPaneHeights() {
    ...
    const a = this.#viewDefaultH * (cnt - 1),
      elRowsH = this.#elRows.height,
      ratio = a / Math.log10(a * 2) / 100,
      rowsH = Math.round(elRowsH * ratio),
      sizes = {};
  ...
  paneMaximize(p) {
    ...
    const elRowsH = this.#elRows.height;
    let style;
    ...
    // maximize pane, minimize the rest
      ...
      maxMin.rowsH = elRowsH;
      for (let [k, v] of this.#ChartPanes.entries()) {
        // store height for restore
        maxMin.panes[k] = v.element.height;
        style = v.element.style;
        if (p === v) {
          style.display = "block";
          v.setDimensions({ w: undefined, h: elRowsH });
  ...
  paneCollapse(p) {
    ...
    if (p.collapsed.state) {
      ...
      else if (pc.rowsHeight !== elRowsH) {
        h = pc.height * (pc.rowsHeight / elRowsH);
      }
      // no changes
      else {
        h = pc.height;
    ...
    // collapse the chart pane
      ...
      h = (p.element.height - COLLAPSEDHEIGHT) / exp.length;
      for (let o of exp) {
        n = o.element.height + h;
        o.setDimensions({ w: undefined, h: n });
      }
