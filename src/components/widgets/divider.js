...
export default class Divider {
  ...
  dividerNode() {
    ...
    const node = `
      <div id="${
        this.#id
      }" class="divider" style="${styleDivider}"><hr style="${styleLine}"></div>
    `;
    return node;
  }
  ...
  setPos() {
    ...
    // let curr = this.#chartPane;
    // let next = true;
    // do {
    //   next = curr.sibling("next");
    //   if (!!next) {
    //     pos += next.height;
    //     curr = next;
    //   }
    // } while (!!next);
    // pos += this.#core.theme.time.height + this.height;
    // this.#elDivider.style.top = `${pos * -1}px`;
  }

  setWidth() {
    this.#elDivider.style.width = `${this.#core.width}px`;
  }
